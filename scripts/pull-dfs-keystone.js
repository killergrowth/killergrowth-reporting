 ...
 * pull-dfs-timnath.js
 *
 * Runs DFS rank tracking for Keystone Painting and writes the result into:
...
const pipelineConfig = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, '../../../tools/hyperlocal-pipeline/clients/keystone-painting.json'),
    'utf8'
  )
);
...
async function run() {
  ...
  console.log(`\nDFS rank pull — Keystone Painting — ${TODAY}`);
